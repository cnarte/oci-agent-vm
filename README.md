...
availability_domain: ""       # blank selects the first AD in the region
fault_domain: ""              # blank lets OCI choose; try FAULT-DOMAIN-1/2/3 if capacity is tight
install_git: true
...
| `fault_domain` | Optional fault domain; blank lets OCI choose |
...
| `enable_provisioning` | `true` creates resources; `false` is discovery-only with a fresh/separate state |
| `telegram.*` | Optional post-provisioning Telegram configuration |

## Read-only discovery

For discovery, use a separate Terraform state and set `enable_provisioning: false` with existing
resource OCIDs. Do not switch a state that already manages this VM from `true` to `false`: Terraform
will interpret the managed resources as removed and plan their destruction. For an already-managed
state, use `terraform plan -refresh-only` when you only want to inspect drift.
